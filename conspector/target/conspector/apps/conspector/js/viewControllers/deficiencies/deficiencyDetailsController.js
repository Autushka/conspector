viewControllers.controller('deficiencyDetailsView', function($scope, $rootScope, $modal, $window, $timeout, $filter, $q, ngTableParams, dataSrv, globalSrv, $state, customSrv, FileUploader) {
    ...
    // $scope.refreshUnitDetails = function() {
    //     if ($scope.oDeficiency.component) {
    //         var readUnitSrv = customSrv.readODataEntityNew({
    //             path: "Component",
    //             key: $scope.oDeficiency.component.rowId,
    //             expand: "project,version"
    //         });

    //         readUnitSrv.then(function(oData) {
    //             $scope.oDeficiency.projectName = oData.project.name;
    //             $scope.oDeficiency.phaseName = oData.version.name;
    //         });
    //     }
    // };
    ...
    $scope.getUnitsByUser = function() {
        customSrv.getEntitySet({
            ...
            fnSuccessCallBack: function(aData) {
                //var sUserName = aData[0].userName;
                if (aData[0]) {
                    customSrv.getEntitySet({ // get Units
                        oReadServiceParameters: {
                            path: "Component",
        ...
        oGetDeficienciesAttachments.then(function(aData) {
            for (var i = 0; i < aData.length; i++) {
                if (aData[i].isDeleted !== true) {
                    if (aData[i].fileExtention === ".jpg" || aData[i].fileExtention === ".JPG") {
                        $scope.oAttachments.aPhotos.push(aData[i]);
                    }
                    if (aData[i].fileExtention === ".pdf") {
    ...
    $scope.prepareDeficiencyForSave = function() {
        ...
        oDeficiencyForSave.description = $scope.oDeficiency.description;
