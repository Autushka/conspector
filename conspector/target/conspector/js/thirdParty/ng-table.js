...
app.factory('ngTableParams', ['$q', '$log', function ($q, $log) {
    ...
    var ngTableParams = function (baseParameters, baseSettings) {
      ...
      //OA 04.09.2014 start
    //     this.onChangeGrouping = function(oGroup, oListTable){
    //   	  oGroup.$hideRows = !oGroup.$hideRows;
    //   	  oListTable.reload();
    //     };
        
        this.setGroupsInfo = function(){
      	  //var iGroupedItems = 0;
      	  settings.$scope.aGroupsInfo = [];
				if (settings.$scope.$groups) {
					for (var i = 0; i < settings.$scope.$groups.length; i++) {
						settings.$scope.aGroupsInfo.push({
							value: settings.$scope.$groups[i].value,
							$hideRows: settings.$scope.$groups[i].$hideRows
						});
						
						// if(settings.$scope.$groups[i].$hideRows){
						// 	iGroupedItems = iGroupedItems + settings.$scope.$groups[i].data.length;
						// }
					}
				}
				
				//return iGroupedItems;
        };
      //OA 04.09.2014 end
        ...
        this.getGroups = function ($defer, column) {
      	  //OA 06.09.2104
      	  // if(settings.$scope.onChangeGrouping === undefined){
         // 	  settings.$scope.onChangeGrouping = this.onChangeGrouping;
      	  // }
            ...
            defer.promise.then(function (data) {
                ...
                for (var i in groups) {
              	  //OA 04.09.2014 start
               	  if(settings.$scope.aGroupsInfo && settings.$scope.aGroupsInfo.length > 0){
	     						for (var j = 0; j < settings.$scope.aGroupsInfo.length; j++) {
	     							if(i === settings.$scope.aGroupsInfo[j].value){
	     								groups[i].$hideRows = settings.$scope.aGroupsInfo[j].$hideRows;
	     							} 
	   						}
               	  }
               	//OA 04.09.2014 end
        ...
        var settings = {
            ...
            //OA 04.09.2014 start
            setGroupsInfo: this.setGroupsInfo,
            //onChangeGrouping: this.onChangeGrouping
            //OA 04.09.2014 end
        };
